
Object.defineProperty(exports, "__esModule", { value: true });

const {
  Decimal,
  objectEnumValues,
  makeStrictEnum
} = require('@prisma/client/runtime/index-browser')


const Prisma = {}

exports.Prisma = Prisma

/**
 * Prisma Client JS version: 4.3.1
 * Query Engine version: c875e43600dfe042452e0b868f7a48b817b9640b
 */
Prisma.prismaVersion = {
  client: "4.3.1",
  engine: "c875e43600dfe042452e0b868f7a48b817b9640b"
}

Prisma.PrismaClientKnownRequestError = () => {
  throw new Error(`PrismaClientKnownRequestError is unable to be run in the browser.
In case this error is unexpected for you, please report it in https://github.com/prisma/prisma/issues`,
)};
Prisma.PrismaClientUnknownRequestError = () => {
  throw new Error(`PrismaClientUnknownRequestError is unable to be run in the browser.
In case this error is unexpected for you, please report it in https://github.com/prisma/prisma/issues`,
)}
Prisma.PrismaClientRustPanicError = () => {
  throw new Error(`PrismaClientRustPanicError is unable to be run in the browser.
In case this error is unexpected for you, please report it in https://github.com/prisma/prisma/issues`,
)}
Prisma.PrismaClientInitializationError = () => {
  throw new Error(`PrismaClientInitializationError is unable to be run in the browser.
In case this error is unexpected for you, please report it in https://github.com/prisma/prisma/issues`,
)}
Prisma.PrismaClientValidationError = () => {
  throw new Error(`PrismaClientValidationError is unable to be run in the browser.
In case this error is unexpected for you, please report it in https://github.com/prisma/prisma/issues`,
)}
Prisma.NotFoundError = () => {
  throw new Error(`NotFoundError is unable to be run in the browser.
In case this error is unexpected for you, please report it in https://github.com/prisma/prisma/issues`,
)}
Prisma.Decimal = Decimal

/**
 * Re-export of sql-template-tag
 */
Prisma.sql = () => {
  throw new Error(`sqltag is unable to be run in the browser.
In case this error is unexpected for you, please report it in https://github.com/prisma/prisma/issues`,
)}
Prisma.empty = () => {
  throw new Error(`empty is unable to be run in the browser.
In case this error is unexpected for you, please report it in https://github.com/prisma/prisma/issues`,
)}
Prisma.join = () => {
  throw new Error(`join is unable to be run in the browser.
In case this error is unexpected for you, please report it in https://github.com/prisma/prisma/issues`,
)}
Prisma.raw = () => {
  throw new Error(`raw is unable to be run in the browser.
In case this error is unexpected for you, please report it in https://github.com/prisma/prisma/issues`,
)}
Prisma.validator = () => (val) => val

/**
 * Shorthand utilities for JSON filtering
 */
Prisma.DbNull = objectEnumValues.instances.DbNull
Prisma.JsonNull = objectEnumValues.instances.JsonNull
Prisma.AnyNull = objectEnumValues.instances.AnyNull

Prisma.NullTypes = {
  DbNull: objectEnumValues.classes.DbNull,
  JsonNull: objectEnumValues.classes.JsonNull,
  AnyNull: objectEnumValues.classes.AnyNull
}

/**
 * Enums
 */
// Based on
// https://github.com/microsoft/TypeScript/issues/3192#issuecomment-261720275
function makeEnum(x) { return x; }

exports.Prisma.AdminOnInventoryScalarFieldEnum = makeEnum({
  userId: 'userId',
  assignedById: 'assignedById',
  assignedAt: 'assignedAt',
  inventoryId: 'inventoryId'
});

exports.Prisma.AdminOnRecipeBookScalarFieldEnum = makeEnum({
  userId: 'userId',
  recipeBookId: 'recipeBookId',
  assignedById: 'assignedById',
  assignedAt: 'assignedAt'
});

exports.Prisma.AdminOnSpecScalarFieldEnum = makeEnum({
  userId: 'userId',
  specId: 'specId',
  assignedById: 'assignedById',
  assignedAt: 'assignedAt'
});

exports.Prisma.GroupIngredientScalarFieldEnum = makeEnum({
  groupId: 'groupId',
  ingredientSpecId: 'ingredientSpecId'
});

exports.Prisma.GroupModScalarFieldEnum = makeEnum({
  userId: 'userId',
  groupId: 'groupId'
});

exports.Prisma.GroupScalarFieldEnum = makeEnum({
  id: 'id',
  groupName: 'groupName',
  dateCreated: 'dateCreated'
});

exports.Prisma.GroupSpecScalarFieldEnum = makeEnum({
  groupId: 'groupId',
  specId: 'specId'
});

exports.Prisma.GroupUserScalarFieldEnum = makeEnum({
  userId: 'userId',
  groupId: 'groupId'
});

exports.Prisma.IngredientScalarFieldEnum = makeEnum({
  id: 'id',
  dateCreated: 'dateCreated',
  name: 'name',
  postedById: 'postedById'
});

exports.Prisma.IngredientSpecScalarFieldEnum = makeEnum({
  id: 'id',
  dateCreated: 'dateCreated',
  name: 'name',
  amount: 'amount',
  unit: 'unit',
  price: 'price',
  source: 'source',
  postedById: 'postedById',
  ingredientId: 'ingredientId',
  inventoryId: 'inventoryId'
});

exports.Prisma.InventoryIngredientSpecScalarFieldEnum = makeEnum({
  inventoryId: 'inventoryId',
  ingredientSpecId: 'ingredientSpecId',
  addedById: 'addedById',
  sharedAt: 'sharedAt'
});

exports.Prisma.InventoryScalarFieldEnum = makeEnum({
  id: 'id',
  name: 'name',
  createdAt: 'createdAt',
  createdById: 'createdById'
});

exports.Prisma.QueryMode = makeEnum({
  default: 'default',
  insensitive: 'insensitive'
});

exports.Prisma.RecipeBookScalarFieldEnum = makeEnum({
  id: 'id',
  name: 'name',
  createdAt: 'createdAt',
  createdById: 'createdById'
});

exports.Prisma.RecipeBookSpecScalarFieldEnum = makeEnum({
  recipeBookId: 'recipeBookId',
  specId: 'specId',
  addedById: 'addedById',
  sharedAt: 'sharedAt'
});

exports.Prisma.RecipeScalarFieldEnum = makeEnum({
  id: 'id',
  createdAt: 'createdAt',
  name: 'name',
  origin: 'origin',
  history: 'history',
  postedById: 'postedById'
});

exports.Prisma.SharedIngredientScalarFieldEnum = makeEnum({
  userId: 'userId',
  ingredientSpecId: 'ingredientSpecId',
  sharedById: 'sharedById',
  sharedAt: 'sharedAt'
});

exports.Prisma.SharedInventoryScalarFieldEnum = makeEnum({
  userId: 'userId',
  inventoryId: 'inventoryId',
  sharedById: 'sharedById',
  sharedAt: 'sharedAt'
});

exports.Prisma.SharedRecipeBookScalarFieldEnum = makeEnum({
  userId: 'userId',
  recipeBookId: 'recipeBookId',
  sharedById: 'sharedById',
  sharedAt: 'sharedAt'
});

exports.Prisma.SharedSpecScalarFieldEnum = makeEnum({
  userId: 'userId',
  specId: 'specId',
  sharedById: 'sharedById',
  sharedAt: 'sharedAt'
});

exports.Prisma.SortOrder = makeEnum({
  asc: 'asc',
  desc: 'desc'
});

exports.Prisma.SpecScalarFieldEnum = makeEnum({
  id: 'id',
  specName: 'specName',
  postedById: 'postedById',
  instructions: 'instructions',
  glassware: 'glassware',
  ice: 'ice',
  recipeId: 'recipeId',
  recipeBookId: 'recipeBookId'
});

exports.Prisma.TouchScalarFieldEnum = makeEnum({
  id: 'id',
  ingredientId: 'ingredientId',
  order: 'order',
  amount: 'amount',
  unit: 'unit',
  specId: 'specId',
  postedById: 'postedById'
});

exports.Prisma.TransactionIsolationLevel = makeStrictEnum({
  ReadUncommitted: 'ReadUncommitted',
  ReadCommitted: 'ReadCommitted',
  RepeatableRead: 'RepeatableRead',
  Serializable: 'Serializable'
});

exports.Prisma.UserDataScalarFieldEnum = makeEnum({
  id: 'id',
  userId: 'userId',
  userBio: 'userBio',
  workPlace: 'workPlace',
  profilePicture: 'profilePicture'
});

exports.Prisma.UserScalarFieldEnum = makeEnum({
  id: 'id',
  userName: 'userName',
  firstName: 'firstName',
  lastName: 'lastName',
  dateJoined: 'dateJoined',
  email: 'email',
  password: 'password'
});


exports.Prisma.ModelName = makeEnum({
  AdminOnSpec: 'AdminOnSpec',
  AdminOnRecipeBook: 'AdminOnRecipeBook',
  AdminOnInventory: 'AdminOnInventory',
  Group: 'Group',
  GroupIngredient: 'GroupIngredient',
  GroupMod: 'GroupMod',
  GroupSpec: 'GroupSpec',
  GroupUser: 'GroupUser',
  Ingredient: 'Ingredient',
  IngredientSpec: 'IngredientSpec',
  Recipe: 'Recipe',
  RecipeBook: 'RecipeBook',
  Inventory: 'Inventory',
  RecipeBookSpec: 'RecipeBookSpec',
  InventoryIngredientSpec: 'InventoryIngredientSpec',
  Spec: 'Spec',
  UserData: 'UserData',
  User: 'User',
  SharedIngredient: 'SharedIngredient',
  SharedRecipeBook: 'SharedRecipeBook',
  SharedInventory: 'SharedInventory',
  SharedSpec: 'SharedSpec',
  Touch: 'Touch'
});

/**
 * Create the Client
 */
class PrismaClient {
  constructor() {
    throw new Error(
      `PrismaClient is unable to be run in the browser.
In case this error is unexpected for you, please report it in https://github.com/prisma/prisma/issues`,
    )
  }
}
exports.PrismaClient = PrismaClient

Object.assign(exports, Prisma)
